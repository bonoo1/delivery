...
class OrdersTest {
    ...
    void test3() {
        // when
        Long restaurantId = registeredRestaurant.id;
        ResponseEntity<FoodTest.FoodDto[]> response = restTemplate.getForEntity(
                "/restaurant/" + restaurantId + "/foods",
                FoodTest.FoodDto[].class);

        // then
        assertEquals(HttpStatus.OK, response.getStatusCode());
        FoodTest.FoodDto[] foodsResponse = response.getBody();
        assertNotNull(foodsResponse);
        assertEquals(3, foodsResponse.length);
        // 음식 1 확인
        FoodTest.FoodDto food1Response = Arrays.stream(foodsResponse)
        ...
        // 음식 2 확인
        FoodTest.FoodDto food2Response = Arrays.stream(foodsResponse)
        ...
        // 음식 3 확인
        FoodTest.FoodDto food3Response = Arrays.stream(foodsResponse)
